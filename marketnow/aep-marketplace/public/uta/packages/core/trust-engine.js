...
class TrustEngine {
    ...
    translate(payload, opts) {
        ...
        if (!fromAdapter)
            throw new Error(`Adapter for '${fromFormat}' not registered`);
        if (!toAdapter)
            throw new Error(`Adapter for '${opts.to}' not registered`);
    ...
    toUTS(payload, fromFormat) {
        ...
        if (!fmt)
            throw new Error('Could not detect format. Specify fromFormat explicitly.');
        const adapter = this.adapters.get(fmt);
        if (!adapter)
            throw new Error(`Adapter for '${fmt}' not registered`);
    ...
    fromUTS(uts, toFormat) {
        ...
        if (!adapter)
            throw new Error(`Adapter for '${toFormat}' not registered`);
        return adapter.toNative(uts);
    }
    /**
     * Issue a credential in multiple formats simultaneously.
     */
    async issueMulti(input, formats) {
        if (!this.issuerKeys) {
